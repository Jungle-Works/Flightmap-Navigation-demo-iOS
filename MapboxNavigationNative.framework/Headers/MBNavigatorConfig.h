              ...
              maneuverApproachPredictionScalingDuration:(float)maneuverApproachPredictionScalingDuration
                              ...
                              voiceInstructionThreshold:(float)voiceInstructionThreshold
                                 defaultArrivalDistance:(float)defaultArrivalDistance
                                                 useEKF:(BOOL)useEKF
                             mppSpeedAveragingMaxPoints:(uint8_t)mppSpeedAveragingMaxPoints
                           mppMatchTraceSpeedMultiplier:(float)mppMatchTraceSpeedMultiplier
                           mppMatchTraceMinimumDistance:(float)mppMatchTraceMinimumDistance
                           mppMatchTraceMaximumDistance:(float)mppMatchTraceMaximumDistance
                             mppMatchTraceMaximumPoints:(uint8_t)mppMatchTraceMaximumPoints
                            mppMatchMinimumSearchRadius:(uint64_t)mppMatchMinimumSearchRadius
                            mppMatchMaximumSearchRadius:(uint64_t)mppMatchMaximumSearchRadius
                                       mppMinimumLength:(float)mppMinimumLength
                                       mppMaximumLength:(float)mppMaximumLength
                           mppBearingChangeMinimumSpeed:(float)mppBearingChangeMinimumSpeed
                                    mppMaxRequestTimeMs:(uint32_t)mppMaxRequestTimeMs
                                     ehMaxRequestTimeMs:(uint32_t)ehMaxRequestTimeMs;
...
/** Use an Extended Kalman Filter to filter the location updates */
@property (nonatomic, readwrite, getter=isUseEKF) BOOL useEKF;

/** MPP speed average: max fix points * */
@property (nonatomic, readwrite) uint8_t mppSpeedAveragingMaxPoints;

/** MPP match trace: speed (m/s) multiplier to determine minimum distance * */
@property (nonatomic, readwrite) float mppMatchTraceSpeedMultiplier;

/** MPP match trace: minimum distance * */
@property (nonatomic, readwrite) float mppMatchTraceMinimumDistance;

/** MPP match trace: maximum distance * */
@property (nonatomic, readwrite) float mppMatchTraceMaximumDistance;

/** MPP match trace: maximum points * */
@property (nonatomic, readwrite) uint8_t mppMatchTraceMaximumPoints;

/** MPP match options: min search radius * */
@property (nonatomic, readwrite) uint64_t mppMatchMinimumSearchRadius;

/** MPP match options: max search radius * */
@property (nonatomic, readwrite) uint64_t mppMatchMaximumSearchRadius;

/** MPP result options: min length * */
@property (nonatomic, readwrite) float mppMinimumLength;

/** MPP result options: max length * */
@property (nonatomic, readwrite) float mppMaximumLength;

/** MPP minimum speed to allow bearing changes * */
@property (nonatomic, readwrite) float mppBearingChangeMinimumSpeed;

/**
 MPP maximum request time in milliseconds
 controls how long we are ready to wait for every mpp request

 */
@property (nonatomic, readwrite) uint32_t mppMaxRequestTimeMs;

/** EH maximum request time in milliseconds * */
@property (nonatomic, readwrite) uint32_t ehMaxRequestTimeMs;
